...
public class SldzOrderController extends BaseCustomCrudRestController<SldzOrder> {
    ...
    public JsonResult keywords(int state, String keyword, Pagination pagination) throws Exception {
        ...
        wrapper.like(orderNumber, keyword).or().like(buyersRandom, keyword).or().like(buyersName, keyword).or().like(logisticsNumber, keyword);
    ...
    @ApiImplicitParams({ @ApiImplicitParam(name = "LogisticsNumber", value = "快递单号", required = true, dataType = "String") })
    ...
    @ApiImplicitParams({ @ApiImplicitParam(name = "couponJson", value = "优惠券json", required = true, dataType = "String") })
    ...
    public JsonResult AwardCompletedCrdersPreview(String orderNumber) throws Exception {
